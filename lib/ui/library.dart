...
class LibraryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (_) => RoutinesBloc(repository: routineRepository, userId: userId, scheduleRepository: scheduleRepository)
            ..add(FetchRoutines()),
        ),
        BlocProvider(
          create: (_) => PartsBloc(repository: partRepository, userId: userId, scheduleRepository: scheduleRepository)
            ..add(FetchParts()),
        ),
      ],
      child: Scaffold(
      ...
      builder: (context, state) {
        if (state is RoutinesLoading) {
          ...
          if (startedRoutines.isEmpty) {
            return const Text('Henüz başlanmış bir rutininiz yok.',
                style: TextStyle(color: Colors.white70));
          }
      ...
      builder: (context, state) {
        if (state is PartsLoading) {
          ...
          if (favoriteParts.isEmpty) {
            return const Text('Favori part\'ınız yok.',
                style: TextStyle(color: Colors.white70));
          }
      ...
      builder: (context, state) {
        if (state is PartsLoading) {
          ...
          if (startedParts.isEmpty) {
            return const Text('Henüz başlanmış bir part\'ınız yok.',
                style: TextStyle(color: Colors.white70));
          }
