...
enum EnsembleNeuralModelState { uninitialized,
  initializing,
  initialized,
  training,
  trained,
...
//EnsembleNeuralModel
class EnsembleNeuralModel extends BaseModel {
  final _logger = Logger('EnsembleNeuralModel');

  EnsembleNeuralModelState _modelState = EnsembleNeuralModelState.uninitialized;
  EnsembleNeuralModelState get modelState => _modelState;
  ...
  void _updateState(EnsembleNeuralModelState newState) {
    if (_modelState == EnsembleNeuralModelState.error && newState != EnsembleNeuralModelState.uninitialized) {
  ...
  Future<void> setup(List<dynamic> trainingData) async {
    try {
      _modelState = EnsembleNeuralModelState.initializing;
      ...
      _modelState = EnsembleNeuralModelState.initialized;
      _logger.info('EnsembleNeuralModel model setup completed');
    } catch (e) {
      _modelState = EnsembleNeuralModelState.error;
      throw AIModelException('EnsembleNeuralModel model setup failed: $e');
    }
  }
  ...
  Future<void> fit(List<dynamic> trainingData) async {
    try {
      if (_modelState != EnsembleNeuralModelState.initialized) {
        throw AIModelException('Model must be initialized before training');
      }

      _modelState = EnsembleNeuralModelState.training;
      final typedData = trainingData.cast<GymMembersTracking>();
      ...
      _modelState = EnsembleNeuralModelState.trained;
    } catch (e) {
      _modelState = EnsembleNeuralModelState.error;
      throw AIModelException('EnsembleNeuralModel training failed: $e');
    }
  }
  ...
  Future<Map<String, dynamic>> getPredictionMetadata(GymMembersTracking input) async {
    return {
      'model_type': 'EnsembleNeuralModel',
  ...
  Future<FinalDataset> predict(GymMembersTracking input) async {
    try {
      if (_modelState != EnsembleNeuralModelState.trained) {
        throw AIModelException('Model is not trained for inference');
      }
      _modelState = EnsembleNeuralModelState.predicting;
      ...
      _modelState = EnsembleNeuralModelState.error;
      throw AIModelException('EnsembleNeuralModel inference failed: $e');
    }
  }
